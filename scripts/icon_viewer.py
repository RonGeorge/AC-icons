...
WINDOW_TITLE = "Demonic Icon Viewer"
...
class IconViewer:
    ...
    def edit_metadata(self, row, col):
        """Edit metadata popup - larger version"""
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT name, keywords, metadata_json FROM ac_icons WHERE icon_id=?", (icon_id,))
            name, keywords, metadata_json = cursor.fetchone()
            
            # Larger edit window
            edit_win = tk.Toplevel(self.root)
            edit_win.title(f"Edit {icon_id}")
            edit_win.geometry("400x300")  # Larger size
            edit_win.minsize(350, 250)   # Minimum size
            ...
            ttk.Label(main_frame, text="Keywords:").grid(row=2, column=0, sticky="ne", padx=5, pady=5)
            keywords_var = tk.StringVar(value=keywords or "")
            keywords_entry = ttk.Entry(main_frame, textvariable=keywords_var, width=30)
            keywords_entry.grid(row=2, column=1, sticky="w", padx=5, pady=5)
            
            # Metadata JSON in scrollable text widget
            ttk.Label(main_frame, text="Metadata JSON:").grid(row=3, column=0, sticky="ne", padx=5, pady=5)
            json_frame = ttk.Frame(main_frame)
            json_frame.grid(row=3, column=1, sticky="nsew", padx=5, pady=5)
            
            json_text = tk.Text(json_frame, wrap=tk.WORD, width=30, height=8)
    ...
    def display_page(self):
        ...
        try:
            ...
            if self.search_var.get():
                query = """
                SELECT icon_id, name, icon_data FROM ac_icons 
                WHERE icon_id LIKE ? OR name LIKE ? OR keywords LIKE ?
                LIMIT ? OFFSET ?
                """
                params = (f"%{self.search_var.get()}%", f"%{self.search_var.get()}%", 
                         f"%{self.search_var.get()}%", COLUMNS * ROWS, offset)
            else:
                query = "SELECT icon_id, name, icon_data FROM ac_icons LIMIT ? OFFSET ?"
                params = (COLUMNS * ROWS, offset)
                
            cursor.execute(query, params)
            items = cursor.fetchall()
            
            for idx, (icon_id, name, icon_data) in enumerate(items):
                row = idx // COLUMNS
                col = idx % COLUMNS
                ...
                text_label.config(text=f"{icon_id}" + (f"\n{name}" if name else ""))
